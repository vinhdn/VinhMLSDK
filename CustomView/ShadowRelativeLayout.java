...
public class ShawdowRelativeLayout extends RelativeLayout {
    ...
    private int buttonColor = 0xfff;
    private int shadowColor = 0x666;


    boolean isShadowColorDefined = false;

    public ShawdowRelativeLayout(Context context) {
        super(context);
        init();
    }

    public ShawdowRelativeLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
        parseAttrs(context, attrs);
    }

    public ShawdowRelativeLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            ...
            if (attr == R.styleable.ShadowButton_shadowEnabled) {
                isShadowEnabled = typedArray.getBoolean(attr, true); //Default is true
            } else if (attr == R.styleable.ShadowButton_buttonColor) {
                mButtonColor = typedArray.getColor(attr, buttonColor);
//                mButtonColor = typedArray.getColor(attr, R.color.default_color);
            } else if (attr == R.styleable.ShadowButton_shadowColor) {
//                mShadowColor = typedArray.getColor(attr, R.color.default_shadow_color);
                mShadowColor = typedArray.getColor(attr, shadowColor);
                isShadowColorDefined = true;
            } else if (attr == R.styleable.ShadowButton_shadowHeight) {
                mShadowHeight = typedArray.getDimensionPixelSize(attr, R.dimen.default_shadow_height);
            } else if (attr == R.styleable.ShadowButton_cornerRadius) {
    ...
    public void setShadowButtonPadding(int left, int top, int right, int bottom) {
        mPaddingLeft = left;
        mPaddingRight = right;
        mPaddingTop = top;
